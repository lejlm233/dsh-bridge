...
import { MefrpManager, deriveVip } from './mefrp-manager.mjs';
import { MefrpApi } from './mefrp-api.mjs';
import { NPM_PACKAGE, GITHUB_REPO, isValidUpgradeVersion, normalizeGithubTag, buildUpgradeSpecs } from './upgrade-source.mjs';
 ...
 */
class BridgeService {
  constructor({ dshPort, proxyPort, home, cloudflaredConfig, customTunnelConfig, lanConfig, mefrpConfig, authManager, onPersist, logger }) {
    ...
    this.mefrpConfig = mefrpConfig ?? { accessToken: '', nodeId: 0, remotePort: 0, autoStart: false };
    ...
    this.mefrp = null;
    this.mefrpState = { phase: 'idle', detail: '' };
    ...
    this.proxy = new ProxyServer({
      ...
      allowedOrigins: () => {
        ...
        try {
          ...
          if (this.mefrp?.url) origins.push(new URL(this.mefrp.url).origin);
  ...
  async getStatus({ adminAuthValid = false } = {}) {
    ...
    const baseMefrpUrl = this.mefrp?.url || null;
    const mefrpUrl = appendToken(baseMefrpUrl, isPublicProtected);
    ...
    return {
      ...
      mefrp: {
        running: this.isMefrpActive(),
        url: mefrpUrl,
        rawUrl: baseMefrpUrl,
        qr: mefrpUrl
          ? await this.qrCache.get(mefrpUrl)
          : null,
        state: this.mefrpState,
        tokenConfigured: !!this.mefrpConfig?.accessToken,
        token: adminAuthValid ? (this.mefrpConfig?.accessToken || '') : (this.mefrpConfig?.accessToken ? '******' : ''),
        nodeId: Number(this.mefrpConfig?.nodeId) || 0,
        remotePort: Number(this.mefrpConfig?.remotePort) || 0,
        autoStart: Boolean(this.mefrpConfig?.autoStart),
        // 当前这次运行创建的代理 ID：前端据此把隧道列表里的「当前」标出来，
        // 并给公网地址那一行挂删除按钮（删除当前代理会连带关闭隧道）。
        proxyId: this.mefrp?.createdProxy?.proxyId ?? null,
      },
  ...
  // 保存 mefrp 公网隧道配置（访问令牌 / 指定节点 / 指定远端端口）。
  // 字段语义对齐 saveCloudflaredConfig：undefined = 客户端未改不上传；
  // '******' = 非管理员视图掩码回显，保留现值，防止真实令牌被掩码覆盖。
  async saveMefrpConfig({ accessToken, nodeId, remotePort } = {}) {
    const prev = this.mefrpConfig ?? {};
    const next = { ...prev };
    if (accessToken !== undefined) next.accessToken = accessToken === '******' ? (prev.accessToken ?? '') : String(accessToken).trim();
    if (nodeId !== undefined) next.nodeId = nodeId ? Number(nodeId) : 0;
    if (remotePort !== undefined) next.remotePort = remotePort ? Number(remotePort) : 0;
    this.mefrpConfig = next;
    await this.onPersist?.({ mefrp: this.mefrpConfig });
  }

  // 拉取 mefrp 节点列表，供用户手动指定节点（不指定时自动挑「在线 + 非 VIP + 负载最低」）。
  // 传入 accessToken 时用「输入框里尚未保存的令牌」查询，方便先选节点再一起保存。
  async listMefrpNodes(accessTokenOverride) {
    const token = String(accessTokenOverride ?? '').trim() || this.mefrpConfig?.accessToken || '';
    if (!token) throw new Error('请先填写 mefrp 访问令牌');

    const api = new MefrpApi({ timeoutMs: 20000 });
    const [rawNodes, rawStatus] = await Promise.all([
      api.getNodeList(token).catch(() => []),
      api.getNodeStatus(token).catch(() => []),
    ]);
    const statusMap = new Map();
    if (Array.isArray(rawStatus)) {
      for (const s of rawStatus) if (s && s.nodeId != null) statusMap.set(s.nodeId, s);
    }

    const nodes = (Array.isArray(rawNodes) ? rawNodes : []).map((n) => {
      const nodeId = Number(n?.nodeId) || 0;
      const st = statusMap.get(nodeId);
      return {
        nodeId,
        name: n?.name || '',
        hostname: n?.hostname || '',
        ...
        allowPort: n?.allowPort || '',
        isOnline: n?.isOnline !== false && !n?.isDisabled,
        vip: deriveVip(n),
        loadPercent: st && st.loadPercent != null ? Number(st.loadPercent) : null,
      };
    });

    // 排序：在线 → 非 VIP → 负载低（把「真正能用」的顶到最前）
    nodes.sort((a, b) => {
      if (a.isOnline !== b.isOnline) return a.isOnline ? -1 : 1;
      if (a.vip !== b.vip) return a.vip ? 1 : -1;
      const la = a.loadPercent == null ? 999 : a.loadPercent;
      const lb = b.loadPercent == null ? 999 : b.loadPercent;
      return la - lb;
    });
  ...
  // 列出该账号 mefrp 后台的**全部**隧道（含历史遗留的孤儿隧道）。
  //
  // 为什么需要：MefrpManager.stop() 会删掉自己创建的代理，但进程被强杀 / 插件重启导致
  // 内存里的 createdProxy 丢失时，代理就永远留在服务端了；免费账号名额有限（默认 2 个），
  // 反复重试几次后台就会堆一批旧隧道，把名额占满（之后 startMefrp 会直接报「名额已满」）。
  // 这里给 UI 一个「看得见、点得掉」的入口，逻辑与 LunaShare 的 getProxyListWithNodes 一致。
  //
  // 传入 accessToken 时用「输入框里尚未保存的令牌」查询（与 listMefrpNodes 同）。
  async listMefrpTunnels(accessTokenOverride) {
    const token = String(accessTokenOverride ?? '').trim() || this.mefrpConfig?.accessToken || '';
    if (!token) throw new Error('请先填写 mefrp 访问令牌');

    const data = await new MefrpApi({ timeoutMs: 20000 }).getProxyListWithNodes(token);

    // data 可能是 { nodes, proxies }、纯数组、或 { list }，逐个兼容（对齐 MefrpManager._findProxy）
    let proxies = [];
    ...
    // 公网地址只能从 proxy/list 里的 nodes 拿：node/list 的 hostname 对普通用户恒为空
    const hosts = {};
    const nodes = Array.isArray(data?.nodes) ? data.nodes : [];
    for (const n of nodes) if (n && n.nodeId != null) hosts[n.nodeId] = n.hostname || '';

    const activeProxyId = this.mefrp?.createdProxy?.proxyId ?? null;

    const tunnels = proxies.map((p) => {
      const proxyId = Number(p?.proxyId) || 0;
      const nodeId = Number(p?.nodeId) || 0;
      const remotePort = Number(p?.remotePort) || 0;
      const host = hosts[nodeId] || '';
      const domain = p?.domain ? String(p.domain) : '';
      return {
        proxyId,
        ...
        nodeHost: host,
        domain,
        // 公网访问地址：优先 domain，其次 nodeHost:remotePort（与 MefrpManager 的算法一致）
        url: domain
          ? (domain.startsWith('http') ? domain : `http://${domain}`)
          : (host && remotePort ? `http://${host}:${remotePort}` : ''),
        // 当前正在使用的那条（前端标「当前」，公网地址那一行据此挂删除按钮）
        active: activeProxyId != null && proxyId === activeProxyId,
      };
    });

    // 当前在用的排最前，其余按 proxyId 倒序（新建的在前）
    tunnels.sort((a, b) => (a.active !== b.active ? (a.active ? -1 : 1) : b.proxyId - a.proxyId));
    return tunnels;
  }

  // 删除 mefrp 隧道（清理历史遗留、或名额占满时的自救入口）。
  //
  // 删的若是当前正在运行的代理，必须**连带停掉隧道**：mefrpc 的 -p 指向已删除的代理，
  // 留着只会无限重连报错，不如直接停干净。停之前先把 createdProxy 置空，
  // 免得 MefrpManager.stop() 再去删一次（那会报「隧道不存在」）。
  async deleteMefrpTunnel({ proxyId, accessToken } = {}) {
    const token = String(accessToken ?? '').trim() || this.mefrpConfig?.accessToken || '';
    if (!token) throw new Error('请先填写 mefrp 访问令牌');
    const id = Number(proxyId) || 0;
    if (!id) throw new Error('缺少隧道 ID');

    const isActive = this.mefrp?.createdProxy?.proxyId === id;
    // 先停隧道再删代理：免得 mefrpc 对着已删除的代理无限重连报错。
    // stop() 现在不会再删代理，所以这里删的正是用户点名要删的那一条。
    if (isActive) await this.stopMefrp();

    await new MefrpApi({ timeoutMs: 20000 }).deleteProxy(token, id);
    return { proxyId: id, wasActive: isActive };
  }
  ...
  // ── mefrp（幻缘映射）公网隧道后端 ────────────────────────────────────────
  // mefrp 与 Cloudflare 的差异：需先「通过 API 创建代理（选节点+端口）→ 拿 proxyId」再
  // 启动 mefrpc 客户端；公网 URL 来自 API（nodeHost:remotePort / domain），非日志解析。
  // 管理器逻辑见 lib/mefrp-manager.mjs，协议层见 lib/mefrp-api.mjs。

  // mefrp 是否处于"活动"状态（对齐 isCloudflaredActive）。
  isMefrpActive() {
    const phase = this.mefrpState?.phase;
    return phase === 'ready' || phase === 'connecting' || phase === 'downloading' || phase === 'reconnecting';
  }

  async startMefrp({ autoStart = true } = {}) {
    if (this.mefrp && this.isMefrpActive()) {
      throw new Error('mefrp 隧道已在运行');
    }
    if (!this.mefrpConfig?.accessToken) {
      throw new Error('请先在控制台填写 mefrp 访问令牌');
    }
    ...
    // 旧 manager（error/自愈中断留的引用）先彻底停止再重建，杜绝双跑
    if (this.mefrp) {
      this.mefrp.stop();
      this.mefrp = null;
    }

    this.mefrpState = { phase: 'connecting', detail: '正在初始化 mefrp...' };
    this.mefrp = new MefrpManager({
      port: this.proxyPort,
      home: this.home,
      accessToken: this.mefrpConfig?.accessToken,
      nodeId: this.mefrpConfig?.nodeId || null,
      remotePort: this.mefrpConfig?.remotePort || null,
      onStateChange: (state) => {
        this.mefrpState = state;
        // 不在 error 时清空 this.mefrp 引用：manager 会自行退避重试（reconnecting），
        // 引用必须保留到用户显式 stop/reset，否则自愈后 UI 失去控制句柄。
      },
      logger: this.logger,
    });

    // 非阻塞启动，进度通过 onStateChange 推送
    this.mefrp.start();
  }

  // 用户显式关闭：停止进程 + 释放 API 代理名额 + 持久化 autoStart=false。
  async stopMefrp() {
    if (this.mefrp) {
      this.mefrp.stop();
      this.mefrp = null;
  ...
  // 仅停止进程与状态，不动持久化配置（dispose 路径用，保留用户的 autoStart 意图）
  _haltMefrp() {
    if (this.mefrp) {
      this.mefrp.stop();
      this.mefrp = null;
  ...
  // 重置 mefrp 隧道：**先删掉服务端代理**（否则复用逻辑会把老地址原样捡回来，
  // 「重置链接」就换不到新地址了）→ 停隧道 → 清掉已下载的 mefrpc 二进制（下次重新下载）。
  async resetMefrp() {
    const mgr = this.mefrp;
    if (mgr) {
      try {
        await mgr.deleteCreatedProxy();
      } catch (err) {
        this.logger?.warn('重置 mefrp：删除旧代理失败（继续重置）: %s', err.message);
      }
    }
    await this.stopMefrp();
    const binDir = join(this.home ?? join(homedir(), '.dsh-bridge'), 'bin');
    const candidates = ['mefrpc.exe', 'mefrpc'];
    for (const name of candidates) {
      const p = join(binDir, name);
      try { await unlink(p); } catch {}
    }
  ...
  async checkVersion() {
    ...
    // 通用 JSON 拉取（npm registry 与 GitHub API 共用）。
    // GitHub API 强制要求 User-Agent，缺失直接 403；不带 token 时限流 60 次/时，
    // 上面那个 10 分钟 TTL 缓存足够避开。
    const fetchJson = (url, timeoutMs = 4000) => new Promise((resolve, reject) => {
      const req = httpsGet(url, {
        timeout: timeoutMs,
        headers: { 'User-Agent': 'dsh-bridge', Accept: 'application/vnd.github+json' },
      }, (res) => {
        if (res.statusCode !== 200) {
          res.resume(); // 非 200 也要把响应流读完，否则 socket 挂着不放
    ...
    // npm 源：优先国内高速镜像 npmmirror，降级 npmjs 官方源
    const fetchNpmLatest = (pkgName) => fetchJson(`https://registry.npmmirror.com/${pkgName}/latest`, 3500)
      .catch(() => fetchJson(`https://registry.npmjs.org/${pkgName}/latest`, 5000));

    // GitHub 兜底源：优先 Release（能顺带拿到 release notes），仓库还没建 Release 时退回 tags。
    // 两条都拿不到就返回 null（不抛）—— 这样上面 npm 那条的错误信息能留下来做诊断。
    const fetchGithubLatest = async () => {
      const rel = await fetchJson(`https://api.github.com/repos/${GITHUB_REPO}/releases/latest`, 5000).catch(() => null);
      const released = normalizeGithubTag(rel?.tag_name);
      if (released) return { version: released, releaseNotes: rel.body ?? null };
      const tags = await fetchJson(`https://api.github.com/repos/${GITHUB_REPO}/tags?per_page=1`, 5000).catch(() => null);
      const tagged = Array.isArray(tags) ? normalizeGithubTag(tags[0]?.name) : null;
      return tagged ? { version: tagged, releaseNotes: null } : null;
    };

    try {
      let npmError = null;
      const npmLatest = await fetchNpmLatest(NPM_PACKAGE)
        .catch((e) => { npmError = e; return null; });
      let latestData = npmLatest?.version
        ? { version: npmLatest.version, releaseNotes: npmLatest.releaseNotes ?? npmLatest.description ?? null }
        : null;
      let source = latestData ? 'npm' : null;
      // npm 上查不到（本仓库是 fork，未发布到 npm）→ 兜底查 GitHub Release / tag
      if (!latestData) {
        const ghLatest = await fetchGithubLatest();
        if (ghLatest?.version) {
          latestData = ghLatest;
          source = 'github';
        }
      ...
      const dshLatestData = await fetchNpmLatest('@deepseek-ai/dsh').catch(() => null);
      ...
      const result = {
        ...
        // 'npm' | 'github' | null：决定一键升级先试哪种 spec，也用于面板文案
        source,
        // 两个源都没拿到版本时，把 npm 那边的错误留下便于诊断；
        // 不用 error 字段是因为前端拿 info.error 当作「横幅整体不可信」的信号。
        sourceError: latestData ? null : (npmError?.message ?? null),
        dshVersion: await this.getDshVersion(),
      ...
      return { current: VERSION, latest: null, error: e.message ?? '检查失败', source: null, dshVersion: await this.getDshVersion(), dshLatest: null, dshUpgradable: false, dshUpgradeReason: '' };
    }
  }

  // 一键直接升级插件（执行 dsh / npx / npm 自动升级，使用安全的参数数组彻底杜绝 shell 注入）
  //
  // 两个来源，按 checkVersion 给出的 source 决定先试哪个，另一个留作兜底：
  //   npm —— 已发布到 npm 时首选（国内走 npmmirror 更快）
  //   git —— 本仓库是 fork、未发布到 npm，只能从 GitHub 装（`github:owner/repo#tag`）
  // ⚠️ 不接受客户端传入的任意 URL / spec：git 地址只由 GITHUB_REPO 常量拼出，版本号仍过
  //    严格正则白名单。下面这段 spawn 开了 shell:true，放开就等于开了个命令注入口。
  async upgradePlugin({ version, source } = {}) {
    const targetVersion = version ? String(version).trim() : 'latest';
    // 严格 SemVer 白名单正则校验（与非法的 shell 元字符互斥）
    if (!isValidUpgradeVersion(targetVersion)) {
      return { ok: false, error: `非法的版本号格式: ${targetVersion}`, version: targetVersion };
    }
    const specs = buildUpgradeSpecs(targetVersion, source);
    const isWin = process.platform === 'win32';
    ...
    // 每个来源依次试 dsh / npx / 裸 npm（dsh CLI 不在 PATH 时靠后两者兜底），
    // 一个来源全部失败就换下一个来源。
    const tasks = specs.flatMap((spec) => [
      { spec, cmd: 'dsh', args: ['plugin', '--profile', 'web', 'add', spec] },
      { spec, cmd: existsSync(siblingNpx) ? siblingNpx : 'npx', args: ['--yes', '@deepseek-ai/dsh', 'plugin', '--profile', 'web', 'add', spec] },
      { spec, cmd: existsSync(siblingNpm) ? siblingNpm : 'npm', args: ['install', spec] },
    ]);
    ...
    for (const task of tasks) {
      try {
        ...
        return { ok: true, command: `${task.cmd} ${task.args.join(' ')}`, output, version: targetVersion, spec: task.spec };
      } catch (err) {
        lastError = err;
      }
    }

    return { ok: false, error: lastError?.message ?? '升级命令执行失败', version: targetVersion, tried: specs };
  }
...
function apply(ctx, config = {}) {
  ...
  loadConfig().then(async (stored) => {
    ...
    if (stored?.mefrp) {
      service.mefrpConfig = stored.mefrp;
      logger.info('dsh-bridge: loaded saved mefrp config (autoStart=%s, tokenConfigured=%s)', Boolean(service.mefrpConfig.autoStart), Boolean(service.mefrpConfig.accessToken));
      if (service.mefrpConfig.autoStart && service.mefrpConfig.accessToken) {
        logger.info('dsh-bridge: auto-starting mefrp tunnel...');
        service.startMefrp({ autoStart: true }).catch((err) => {
          logger.error('dsh-bridge: mefrp auto-start failed: %s', err?.message ?? err);
        });
      }
    }
  ...
  const disposeRpc = installBridgeRpc(ctx, {
    ...
    importBackup: async (backup) => {
      ...
      if (incoming.mefrp) {
        service.mefrpConfig = incoming.mefrp;
      }
