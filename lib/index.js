...
class BridgeService {
  ...
  async checkVersion() {
    ...
    try {
      const latestData = await fetchRegistry('https://registry.npmmirror.com/@lejlm233/dsh-bridge/latest', 3500)
        .catch(() => fetchRegistry('https://registry.npmjs.org/@lejlm233/dsh-bridge/latest', 5000));
  ...
  async upgradePlugin({ version } = {}) {
    ...
    const pkgSpec = `@lejlm233/dsh-bridge@${targetVersion}`;
    const isWin = process.platform === 'win32';
