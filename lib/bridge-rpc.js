  ...
  return registerRpcChannel(
    ...
    async (endpoint, payload = {}, signal) => {
      ...
      try {
        ...
        if (endpoint === BRIDGE_ENDPOINTS.saveMefrpConfig) {
          const adminErr = checkAdminAuth(authManager, payload, { requireConfigured: true });
          if (adminErr) return adminErr;

          const { accessToken, nodeId, remotePort } = payload;
          await service.saveMefrpConfig({ accessToken, nodeId, remotePort });
          const status = await service.getStatus();
          return ok(status);
        }

        if (endpoint === BRIDGE_ENDPOINTS.startMefrp) {
          const adminErr = checkAdminAuth(authManager, payload, { requireConfigured: true });
          if (adminErr) return adminErr;

          try {
            await service.startMefrp();
            const status = await service.getStatus();
            return ok(status);
          } catch (err) {
            logger.error('Failed to start mefrp: %s', err.message);
            return fail('bad-request', err.message);
          }
        }

        if (endpoint === BRIDGE_ENDPOINTS.stopMefrp) {
          const adminErr = checkAdminAuth(authManager, payload);
          if (adminErr) return adminErr;

          await service.stopMefrp();
          const status = await service.getStatus();
          return ok(status);
        }

        if (endpoint === BRIDGE_ENDPOINTS.resetMefrp) {
          const adminErr = checkAdminAuth(authManager, payload);
          if (adminErr) return adminErr;

          await service.resetMefrp();
          const status = await service.getStatus();
          return ok(status);
        }

        if (endpoint === BRIDGE_ENDPOINTS.listMefrpNodes) {
          const adminErr = checkAdminAuth(authManager, payload);
          if (adminErr) return adminErr;

          // 节点列表随 status 一起回传（客户端 act() 会用返回值整体替换 status）
          const nodes = await service.listMefrpNodes(payload?.accessToken);
          const status = await service.getStatus();
          return ok({ ...status, mefrpNodes: nodes });
        }

        if (endpoint === BRIDGE_ENDPOINTS.listMefrpTunnels) {
          const adminErr = checkAdminAuth(authManager, payload);
          if (adminErr) return adminErr;

          try {
            // 同样随 status 回传：客户端 act() 会用返回值整体替换 status
            const tunnels = await service.listMefrpTunnels(payload?.accessToken);
            const status = await service.getStatus();
            return ok({ ...status, mefrpTunnels: tunnels });
          } catch (err) {
            logger.error('Failed to list mefrp tunnels: %s', err.message);
            return fail('bad-request', err.message);
          }
        }

        if (endpoint === BRIDGE_ENDPOINTS.deleteMefrpTunnel) {
          const adminErr = checkAdminAuth(authManager, payload);
          if (adminErr) return adminErr;

          try {
            await service.deleteMefrpTunnel({
            ...
            logger.error('Failed to delete mefrp tunnel: %s', err.message);
            return fail('bad-request', err.message);
          }
        }
