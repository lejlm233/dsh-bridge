...
export class MefrpManager {
  ...
  async _ensureProxy() {
    ...
    if (userInfo && userInfo.maxProxies != null && userInfo.usedProxies != null && userInfo.usedProxies >= userInfo.maxProxies) {
      throw new Error(`mefrp 隧道名额已满（${userInfo.usedProxies}/${userInfo.maxProxies}），请先在 mefrp 后台删除多余隧道`);
    }
    ...
    // 4. 选远端端口（未指定时：节点 allowPort / createProxyData.allowPort 随机）
    ...
    // 5. 创建代理（TCP 隧道映射本地 DSH 端口）
    const proxyName = `dsh-bridge-${this.port}-${Date.now() % 100000}`;
    ...
    const proxyId = await this._findProxyId(proxyName);
    ...
    try {
      ...
      const pd = await this._findProxy(proxyName);
  ...
  stop() {
    ...
    // 删除已创建代理，释放 mefrp 隧道名额（免费用户名额有限）
    if (this.createdProxy && this.accessToken) {
      const pid = this.createdProxy.proxyId;
      this.api.deleteProxy(this.accessToken, pid)
        .then(() => this.logger?.info('已删除 mefrp 代理 #%d', pid))
        .catch((e) => this.logger?.warn('删除 mefrp 代理 #%d 失败: %s', pid, e.message));
